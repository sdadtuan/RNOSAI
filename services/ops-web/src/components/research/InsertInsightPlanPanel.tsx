...
export function InsertInsightPlanPanel({
  planId,
  researchJson,
  user,
  onInserted,
  ...
  const snapshot = useMemo(() => parsePlanInsightSnapshot(researchJson), [researchJson]);
  const canView = isMarketResearchFeEnabled() && hasCap(user, 'crm_research', 'view');
  const canInsert = hasCap(user, 'crm_research', 'edit') && hasCap(user, 'crm_mktplan', 'edit');
  ...
  async function onInsert() {
    ...
    setError('');
    try {
      const out = await insertPlanInsights(token, planId, {
        client_id: clientId,
  ...
  return (
    <section className="card" style={{ padding: '1rem', display: 'grid', gap: '0.75rem' }}>
      <h3 style={{ margin: 0, fontSize: '1rem' }}>Chèn insight đã duyệt</h3>
