...
import { fetchLeadPresalesConsultBrief, postLeadPresalesConsultPrefill } from '@/lib/api';
...
export function PresalesConsultBriefPanel({ token, user, leadId, onPrefilled }: Props) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      setBrief(out.brief as ConsultBrief);
      setError('');
    } catch (err) {
  ...
  async function onPrefill() {
    ...
    setError('');
    try {
      const out = await postLeadPresalesConsultPrefill(token, leadId, { overwrite });
      setMessage(`Prefill: ${Number(out.filled ?? 0)} field · ${out.fields?.join(', ') || '—'}`);
      onPrefilled?.();
      await reload();
    } catch (err) {
