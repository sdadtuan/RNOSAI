...
test.describe('CSD chat workspace', () => {
  ...
  test('C-5: launcher opens full chat dialog, hide on chat page', async ({ page }) => {
    await mockCsdChatApis(page);
    await loginAsStaff(page);
    await page.goto('/crm/csd');
    await expect(page.getByTestId('csd-chat-launcher')).toBeVisible({ timeout: 15_000 });
    await page.getByTestId('csd-chat-launcher').click();
    await expect(page).toHaveURL(/\/crm\/csd$/);
    await expect(page.getByTestId('csd-chat-dock')).toBeVisible();
    await unlockCsdChat(page);
    await expect(page.getByTestId('csd-chat-dock').getByTestId('csd-chat-workspace')).toBeVisible({
      timeout: 15_000,
    });
    await page.getByTestId('csd-chat-dock').getByTestId('csd-chat-list').locator('button').first().click();
    await expect(
      page.getByTestId('csd-chat-dock').locator('.csd-chat-message.is-mine, .csd-chat-message.is-theirs'),
    ).toHaveCount(1);
