...
(() => {
  ...
  var SplashScreen = registerPlugin("SplashScreen", {
    web: () => Promise.resolve().then(() => (init_web2(), web_exports2)).then((m) => new m.SplashScreenWeb())
  });
  ...
  // src/deep-link.ts
  function resolvePortalDeepLink(raw) {
  ...
  function resolvePttadsPath(host, pathname, params) {
    ...
    if (segment === "approve" || rest[0] === "approve") {
      const creativeId = rest[0] === "approve" ? rest[1] : rest[0] === void 0 && host !== "local" ? host : rest[0];
  ...
  function navigatePortalDeepLink(raw) {
  ...
  function installFetchClientHeader() {
  ...
  async function bootstrapMobileShell() {
