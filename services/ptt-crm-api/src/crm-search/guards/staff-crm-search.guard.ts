...
@Injectable()
export class StaffCrmSearchViewGuard implements CanActivate {
  ...
  protected async checkCap(context: ExecutionContext, action: 'view' | 'configure'): Promise<boolean> {
    ...
    if (!this.staffAuth.hasCap(me.caps, 'crm_search', action)) {
...
@Injectable()
export class StaffCrmSearchConfigureGuard extends StaffCrmSearchViewGuard {
