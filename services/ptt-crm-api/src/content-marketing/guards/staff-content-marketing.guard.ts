...
@Injectable()
export class StaffContentMarketingQaGuard implements CanActivate {
  constructor(private readonly staffAuth: StaffAuthService) {}
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!this.staffAuth.hasCap(me.caps, 'crm_content', 'qa')) {
      throw new ForbiddenException({ error: 'missing_cap', section: 'crm_content', action: 'qa' });
    }
    return true;
  }
}
...
@Injectable()
export class StaffContentMarketingExecuteGuard implements CanActivate {
  constructor(private readonly staffAuth: StaffAuthService) {}
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!this.staffAuth.hasCap(me.caps, 'crm_content', 'publish')) {
      throw new ForbiddenException({ error: 'missing_cap', section: 'crm_content', action: 'publish' });
    }
    return true;
  }
}
