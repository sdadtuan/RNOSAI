...
@Injectable()
export class StaffInvoicesViewGuard implements CanActivate {
...
@Injectable()
export class StaffInvoicesWriteGuard implements CanActivate {
  constructor(private readonly staffAuth: StaffAuthService) {}
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!req.staffUser) throw new UnauthorizedException({ error: 'Unauthorized' });
    const me = await this.staffAuth.me(req.staffUser);
    if (!this.staffAuth.hasCap(me.caps, 'crm_business_dashboard', 'edit')) {
      throw new ForbiddenException({ error: 'missing_cap', section: 'crm_business_dashboard' });
    }
    return true;
  }
