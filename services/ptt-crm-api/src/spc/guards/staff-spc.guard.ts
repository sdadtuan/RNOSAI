...
@Injectable()
export class StaffSpcViewGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    >();
    return checkCap(this.staffAuth, req, 'spc', 'view', [
...
@Injectable()
export class StaffSpcEditGuard implements CanActivate {
  constructor(private readonly staffAuth: StaffAuthService) {}
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const req = context.switchToHttp().getRequest<
      Request & { staffUser?: StaffJwtPayload; staffAuthVia?: 'internal' | 'jwt' }
    >();
    return checkCap(this.staffAuth, req, 'spc', 'edit', [
...
@Injectable()
export class StaffSpcPublishGuard implements CanActivate {
  constructor(private readonly staffAuth: StaffAuthService) {}
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const req = context.switchToHttp().getRequest<
      Request & { staffUser?: StaffJwtPayload; staffAuthVia?: 'internal' | 'jwt' }
    >();
    return checkCap(this.staffAuth, req, 'spc', 'publish', [
      { section: 'crm_data_config', action: 'configure' },
    ]);
  }
}
