...
export class CsdChatAccountsService {
  ...
  async login(
    ...
    const row = await this.repo.findByStaffId(actor.staffId);
    if (!row?.enabled || !row.username || !row.password_hash) {
      throw new UnauthorizedException({ error: 'invalid_chat_credentials' });
    }
    if (normalizeUsername(row.username) !== username || !verifyPortalPassword(password, row.password_hash)) {
      throw new UnauthorizedException({ error: 'invalid_chat_credentials' });
    }
    return { ok: true, staff_id: row.staff_id, username: row.username };
  }
  ...
  async upsert(
    ...
    const row = await this.repo.upsert({
      ...
      chat_password: password || undefined,
    ...
    await this.audit.insert({
      ...
      after_json: {
        ...
        created_by_staff_id: admin.staffId,
        chat_password_set: Boolean(password),
      },
    });
    return safe;
  }
