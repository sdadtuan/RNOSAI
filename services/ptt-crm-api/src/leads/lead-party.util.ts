export const LEAD_PARTY_LOGO_MAX_BYTES = 2 * 1024 * 1024;

export const LEAD_PARTY_LOGO_MIMES = ['image/png', 'image/jpeg', 'image/webp'] as const;
...
export function isLeadPartyLogoMimeAllowed(mime: string): boolean {
  return (LEAD_PARTY_LOGO_MIMES as readonly string[]).includes(String(mime ?? '').trim().toLowerCase());
}

export function leadPartyLogoExt(mime: string): 'png' | 'jpg' | 'webp' | null {
...
export function leadPartyFromCreateBody(raw: unknown): LeadPartyFields | null {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;
  const body = raw as Record<string, unknown>;
  const hasAny =
    'company_name' in body ||
    'company_address' in body ||
    'phone' in body ||
    'email' in body ||
    'logo_asset_id' in body;
  if (!hasAny) return null;
  return {
    company_name: body.company_name == null ? undefined : String(body.company_name),
    company_address: body.company_address == null ? undefined : String(body.company_address),
    phone: body.phone == null ? undefined : String(body.phone),
    email: body.email == null ? undefined : String(body.email),
    logo_asset_id: body.logo_asset_id == null ? undefined : String(body.logo_asset_id),
  };
}
