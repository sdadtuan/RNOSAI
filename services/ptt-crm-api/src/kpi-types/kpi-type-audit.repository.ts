...
import {
  ...
  type KpiTypeAuditRow,
} from './kpi-types.types';

export type KpiTypeAuditInsert = {
  entity_id: string;
  action: string;
...
@Injectable()
export class KpiTypeAuditRepository implements OnModuleDestroy {
  ...
  async insert(input: KpiTypeAuditInsert): Promise<void> {
    await this.db.query(
      `INSERT INTO crm_kpi_type_audit_logs (
  ...
  async listByEntity(
    entityId: string,
    query: KpiTypeAuditQuery,
  ): Promise<{ rows: KpiTypeAuditRow[]; total: number }> {
