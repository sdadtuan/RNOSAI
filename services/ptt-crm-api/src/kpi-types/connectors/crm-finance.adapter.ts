...
export class CrmFinanceKpiTypeAdapter implements KpiTypeDataSourceAdapter {
  readonly adapterKey = 'crm_finance';
  ...
  async preview(ast: KpiFormulaAst, period: KpiTypePreviewPeriod): Promise<KpiTypePreviewResult> {
    if (ast.entity !== 'AttributedRevenue') {
    ...
    const agg = ast.aggregation === 'AVG' ? 'AVG(conversion_value)' : 'SUM(conversion_value)';
