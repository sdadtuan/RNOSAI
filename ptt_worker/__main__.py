...
def run_worker(*, poll_interval: float = 1.0, once: bool = False) -> int:
    ...
    while _running:
        ...
        try:
            if job_type == "ingest_lead":
            ...
            elif job_type == "research_deep_research":
                from ptt_jobs.handlers.research_deep import run_research_deep_job

                run_research_deep_job(job)
            elif job_type == "research_triangulate":
                from ptt_jobs.handlers.research_triangulate import run_research_triangulate_job

                run_research_triangulate_job(job)
            elif job_type == "research_pulse":
                from ptt_jobs.handlers.research_pulse import run_research_pulse_job

                run_research_pulse_job(job)
            elif job_type == "research_whisper_ingest":
            ...
            elif job_type == "research_qualtrics":
                ...
                run_pipeline_risk_scan_job(job)
            elif job_type == "forecast_snapshot":
            ...
            elif job_type == "churn_health_scan":
