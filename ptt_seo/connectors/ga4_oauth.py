"""Google Analytics 4 OAuth (Phase 4 — PG-only token storage)."""
...
def _client_config() -> tuple[str, str, str]:
    client_id = (
        os.environ.get("PTT_GA4_OAUTH_CLIENT_ID")
        or os.environ.get("PTT_GSC_OAUTH_CLIENT_ID")
        or os.environ.get("PTT_GOOGLE_ADS_CLIENT_ID")
        or ""
    ).strip()
    client_secret = (
        os.environ.get("PTT_GA4_OAUTH_CLIENT_SECRET")
        or os.environ.get("PTT_GSC_OAUTH_CLIENT_SECRET")
        or os.environ.get("PTT_GOOGLE_ADS_CLIENT_SECRET")
        or ""
    ).strip()
    redirect_uri = (os.environ.get("PTT_GA4_OAUTH_REDIRECT_URI") or "").strip()
    if not redirect_uri:
        gsc_uri = (os.environ.get("PTT_GSC_OAUTH_REDIRECT_URI") or "").strip()
        if gsc_uri and "/gsc/oauth/callback" in gsc_uri:
            redirect_uri = gsc_uri.replace("/gsc/oauth/callback", "/ga4/oauth/callback")
        else:
            redirect_uri = (os.environ.get("PTT_GOOGLE_OAUTH_REDIRECT_URI") or "").strip()
    if not client_id or not client_secret or not redirect_uri:
        raise ValueError("missing_ga4_oauth_env")
    return client_id, client_secret, redirect_uri


def build_oauth_state(*, customer_id: int, property_id: str = "") -> str:
    payload = {
...
def authorization_url(*, customer_id: int, property_id: str = "") -> str:
    cid, _, redirect_uri = _client_config()
    state = build_oauth_state(customer_id=customer_id, property_id=property_id)
    params = {
        "client_id": cid,
...
def resolve_ga4_refresh_token(customer_id: int) -> str | None:
    ga4 = get_ga4_integration(customer_id)
    enc = ga4.get("refresh_token_encrypted")
    if enc:
        ...
        try:
            ...
            return None
    dev = (os.environ.get("PTT_GA4_REFRESH_TOKEN") or "").strip()
    return dev or None


def save_ga4_oauth_tokens(
    customer_id: int,
    tokens: dict[str, Any],
    *,
    property_id: str = "",
    ...
    patch_integrations(customer_id, {"ga4": ga4})
    return get_ga4_integration(customer_id)
