#!/usr/bin/env python3
"""Unit tests — Wave 5+++ CRM gates (RE staff/lead-config/workflow/export phase)."""
...
class Wave5PppGateTests(unittest.TestCase):
    _WAVE5_PPP_ENV = {
    ...
    def test_wave5_ppp_config_flags(self) -> None:
        with patch.dict(os.environ, self._WAVE5_PPP_ENV, clear=False):
            self.assertTrue(re_projects_ops_on_nest())

    def test_wave5_ppp_gates_pass(self) -> None:
        with patch.dict(os.environ, self._WAVE5_PPP_ENV, clear=False):
            report = run_gates()
        self.assertTrue(report["ok"], msg=str(report.get("failed_ids")))
        ids = [c["id"] for c in report["checks"]]
        self.assertIn("W5PPP-G01", ids)
        self.assertIn("W5PPP-G02", ids)
        self.assertIn("W5PPP-G03", ids)

    def test_wave5_ppp_registry_notes(self) -> None:
        ...
        self.assertIn("lead-config", notes)
