"""Wave 5+ — RE projects accounting phase gates."""
...
def _check_wave5_base() -> dict[str, Any]:
...
def _check_re_projects_accounting_nest() -> dict[str, Any]:
    ...
    ok = "accounting/dashboard" in text and "accounting/cash-flow" in text
    return {"id": "W5P-G01", "ok": ok, "label": "Nest re-projects accounting routes"}


def _check_ops_web_accounting() -> dict[str, Any]:
    ...
    ok = "fetchReProjectAccountingDashboard" in api_text and "accounting" in page_text
    return {"id": "W5P-G02", "ok": ok, "label": "ops-web RE accounting UI"}


def _check_registry_accounting_retired() -> dict[str, Any]:
    ...
    return {"id": "W5P-G03", "ok": ok, "label": "Registry re_projects RETIRED with Nest routes"}


def _check_wave5p_flags() -> dict[str, Any]:
    from ptt_crm.config import re_projects_accounting_on_nest

    expect = _truthy("WAVE5P_EXPECT_ACCOUNTING_NEST", "1")
    actual = re_projects_accounting_on_nest()
    return {
        "id": "W5P-G04",
        "ok": actual == expect,
        "label": "Wave 5+ RE accounting on Nest",
        "actual": actual,
        "expected": expect,
    }
...
def run_gates() -> dict[str, Any]:
    checks = [
        _check_wave5_base(),
        _check_re_projects_accounting_nest(),
        _check_ops_web_accounting(),
        _check_registry_accounting_retired(),
        _check_wave5p_flags(),
    ]
    ok = all(c.get("ok") for c in checks)
    report = {
        "wave": "5+",
        "component": "crm_re_projects_accounting",
        "ok": ok,
    ...
    dest = _artifacts_dir() / "wave5-plus-gate-report.json"
