"""Phase 5 prod soak evidence — governance / portal / experiments health snapshots."""
...
DEFAULT_SOAK_LOG = Path(".local-dev/phase5-soak-evidence.jsonl")
DEFAULT_REQUIRED_DAYS = 7
DEFAULT_MIN_SAMPLES = 7


def soak_log_path() -> Path:
    raw = (os.environ.get("PTT_PHASE5_SOAK_LOG") or "").strip()
...
def collect_snapshot(*, hours: int = 24) -> dict[str, Any]:
    ...
    snap: dict[str, Any] = {
        ...
        "ok": True,
        "flags": {
            "PTT_SEO_GOVERNANCE_ENABLED": governance_enabled(),
            "PTT_SEO_EXPERIMENTS_ENABLED": experiments_enabled(),
            "PTT_PORTAL_SEO_ENABLED": portal_seo_enabled(),
        },
        "metrics": {},
    }
    since = (_utc_now() - timedelta(hours=max(1, hours))).strftime("%Y-%m-%d %H:%M:%S")
    try:
        ...
        with seo_read() as conn:
            eval_row = conn.execute(
                """
                SELECT COUNT(*) AS n FROM seo_governance_evaluations
                WHERE evaluated_at >= ?
                """,
                (since,),
            ).fetchone()
            blocked_row = conn.execute(
                """
                SELECT COUNT(*) AS n FROM seo_governance_evaluations
                WHERE evaluated_at >= ? AND NOT passed
                """,
                (since,),
            ).fetchone()
            exp_row = conn.execute(
                "SELECT COUNT(*) AS n FROM seo_experiments WHERE status = 'running'"
            ).fetchone()
            portal_row = conn.execute(
                "SELECT COUNT(*) AS n FROM seo_portal_client_map WHERE active IS NOT FALSE"
            ).fetchone()
            snap["metrics"] = {
                "governance_evaluations_24h": int(dict(eval_row)["n"]),
                "governance_blocked_24h": int(dict(blocked_row)["n"]),
                "experiments_running": int(dict(exp_row)["n"]),
                "portal_maps_active": int(dict(portal_row)["n"]),
            }
...
def evaluate_soak_gate(
    *,
    ...
    required_days = float(required_days if required_days is not None else os.environ.get("PTT_PHASE5_SOAK_DAYS", DEFAULT_REQUIRED_DAYS))
    min_samples = int(min_samples if min_samples is not None else os.environ.get("PTT_PHASE5_SOAK_MIN_SAMPLES", DEFAULT_MIN_SAMPLES))
    ...
    if not timestamps:
        return {"ok": False, "label": "Phase 5 soak", "error": "invalid_timestamps"}
