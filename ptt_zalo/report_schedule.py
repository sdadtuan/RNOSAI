"""Scheduled Zalo client reports — public.zalo_report_schedules (Prod-S2)."""
...
from ptt_jobs.channel_hub_fetch import fetch_zalo_hub_for_client, portal_performance_link
...
SCHEDULE_TABLE = "zalo_report_schedules"
RUNS_TABLE = "zalo_report_schedule_runs"
...
def _build_attachment(hub: dict[str, Any], export_format: str, label: str) -> tuple[Any, str]:
    ...
    if fmt == "csv":
        ...
        return io.BytesIO(content), f"zalo-hub-{hub.get('date_from')}_{hub.get('date_to')}.csv"
    pdf_buf, filename = build_zalo_hub_pdf(hub, customer_label=label)
...
def run_schedule(schedule_id: str) -> dict[str, Any]:
    ...
    hub = fetch_zalo_hub_for_client(client_id=schedule["client_id"], window_days=schedule["window_days"])

    try:
        attachment, filename = _build_attachment(hub, schedule["export_format"], label)
        portal_link = portal_performance_link(schedule["client_id"], "zalo") if schedule["portal_link_enabled"] else ""
        subject = f"[PTT Zalo] Báo cáo {schedule['export_format'].upper()} — {label}"
        body = f"Báo cáo Zalo Ads cho {label} ({hub.get('date_from')} → {hub.get('date_to')})."
        if portal_link:
        ...
        logger.exception("zalo report schedule failed: %s", exc)
...
def run_due_schedules(*, as_of: str | None = None) -> dict[str, Any]:
    ...
    results = [run_schedule(item["id"]) for item in due]
