"""Zalo Ads insights → daily_performance sync (Wave Z1)."""
...
from ptt_zalo.ads_api import fetch_campaign_insights, zalo_ads_stub_mode, stub_campaign_insights
...
def zalo_insights_sync_enabled() -> bool:
    return _truthy("PTT_ZALO_INSIGHTS_SYNC", "0")


def pg_zalo_insights_ready() -> bool:
    try:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    WHERE table_schema = 'public'
                      AND table_name IN ('daily_performance', 'zalo_insights_sync_state')
                    """
                )
                return int(cur.fetchone()[0] or 0) >= 2
    except Exception as exc:
        logger.debug("pg_zalo_insights_ready: %s", exc)
        return False
...
def _load_zalo_accounts(*, client_id: str | None = None) -> list[dict[str, Any]]:
    ...
    *,
    ...
    with pg_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                UPDATE zalo_insights_sync_state
...
def sync_account_insights(
    ...
    *,
    ...
    account_id = normalize_account_id(str(account.get("external_account_id") or ""))
    if not account_id:
        return {"ok": False, "error": "missing_account_id", "upserted": 0}

    day = target_date.isoformat()
    if stub or zalo_ads_stub_mode():
        rows = stub_campaign_insights(since=day, until=day, account_id=account_id)
        fetch_error = None
    else:
        access_token = resolve_zalo_access_token(account)
        if not access_token:
...
def sync_zalo_insights(
    *,
    target_date: date | str | None = None,
    client_id: str | None = None,
    compute_metrics: bool = True,
) -> dict[str, Any]:
    if not zalo_insights_sync_enabled() and not zalo_ads_stub_mode():
        return {"ok": True, "skipped": True, "reason": "disabled"}
    if not pg_zalo_insights_ready():
        return {"ok": False, "error": "pg_zalo_insights_not_ready"}

    perf_date = _target_date(target_date)
    accounts = _load_zalo_accounts(client_id=client_id)
    if not accounts:
        _update_sync_state(ok=True, accounts_total=0, accounts_failed=0, rows_upserted=0, last_error=None)
        return {"ok": True, "skipped": True, "reason": "no_zalo_accounts", "performance_date": perf_date.isoformat()}

    stub = zalo_ads_stub_mode()
    total_upserted = 0
    ...
    for account in accounts:
        ...
        if outcome.get("ok"):
            ...
            logger.warning("zalo insights sync account failed: %s", failed[-1])
    ...
    if compute_metrics and total_upserted > 0:
        try:
            ...
            logger.exception("Metrics compute after zalo sync: %s", exc)
