#!/usr/bin/env bash
# Deploy Market Research OS P3 — P0+P1+P2+P3 DDL + API + ops-web + portal-web + worker.
#
# Ordering: P3 DDL MUST apply before api / ops-web / portal-web / worker restart.
# portal_visible / waves / decisions must exist before the new API starts.
# Forward path already applies DDL as 1/5 and worker as 5/5 — do not change that order.
# Step 1/5: P0 + P1 + P2 + P3 DDL (P3 last, still before npm/build/restarts).
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p3_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull origin main && bash scripts/deploy_market_research_p3_vps.sh --local
#
# Do NOT flip flags by default (P0 already on). Explicit opt-in only:
#   bash scripts/deploy_market_research_p3_vps.sh --local --enable-flags
set -euo pipefail
...
run_local() {
  echo "== Market Research P3 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 1/5 Apply Market Research P0 + P1 + P2 + P3 DDL (P3 DDL before restarts) =="
  ...
  echo "== Market Research P3 deploy complete =="
  echo "Flags not flipped unless --enable-flags. UAT: bash scripts/smoke_market_research_p3.sh"
}
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p3_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p3_vps.sh --local"
