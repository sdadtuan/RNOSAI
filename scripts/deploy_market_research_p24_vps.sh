#!/usr/bin/env bash
# Deploy Market Research OS P24 — P0–P23 stack + portal report-detail stale banner.
#
# Forward path: 1/3 DDL (P0–P7 + P10 + P11 + P13 + P20 fail-soft + P21 + P23) → 2/3 api → 3/3 portal-web.
# P24 rebuilds portal-web (like P19); ops-web unchanged. No P24 DDL.
#
...
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p24_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p24_vps.sh --local
set -euo pipefail
...
run_local() {
  echo "== Market Research P24 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== Market Research P24 deploy complete =="
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p24_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p24_vps.sh --local"
fi
