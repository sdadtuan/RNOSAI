#!/usr/bin/env bash
# RNOS-11 / UI-R2-07 — Playwright global search smoke
set -euo pipefail
...
PY
  ...
  if [[ -z "$TOKEN" ]]; then
    ...
    SEARCH_CODE="$(curl -s -o /dev/null -w '%{http_code}' \
      ...
      "${OPS_E2E_API_URL}/api/v1/search/health" || echo 000)"
    if [[ "$SEARCH_CODE" != "200" ]]; then
      echo "WARN  search health returned HTTP $SEARCH_CODE — restarting Nest API"
      NEED_API_START=1
    else
      NEED_API_START=0
      echo "OK  Nest API already running (search health HTTP $SEARCH_CODE)"
    fi
  fi
fi
...
if [[ "${NEED_API_START:-0}" == "1" ]]; then
  ...
  sleep 1
  echo "==> Start ptt-crm-api for RNOS-11 E2E"
  (
