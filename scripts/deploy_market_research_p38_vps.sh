#!/usr/bin/env bash
# Deploy Market Research OS P38 — persist conjoint what-if (RES-UC-100).
#
# Forward path: 1/3 DDL (P0–P38) → 2/3 api → 3/3 ops-web.
# P38 does NOT rebuild portal-web.
# Flags untouched (prod-safe).
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p38_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p38_vps.sh --local
set -euo pipefail
...
run_local() {
  echo "== Market Research P38 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== Market Research P38 deploy complete =="
  echo "UAT: bash scripts/smoke_market_research_p38.sh"
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p38_vps.sh --local"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p38_vps.sh --local"
fi
