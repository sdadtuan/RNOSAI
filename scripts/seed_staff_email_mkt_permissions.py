#!/usr/bin/env python3
"""Seed crm_email_mkt section permissions for marketing positions in PostgreSQL."""
...
EMAIL_MKT_CAPS: dict[str, list[str]] = {
    "MKT-01": ["view", "write", "settings", "compliance", "approve", "deliverability", "reports"],
    "MKT-02": ["view", "write", "reports"],
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Seed staff PG permissions for crm_email_mkt")
    ...
    if not positions:
        print("No MKT positions found in SQLite", file=sys.stderr)
        return 1

    rows: list[tuple[int, str, str]] = []
    for code, caps in EMAIL_MKT_CAPS.items():
        pid = positions.get(code)
        if pid is None:
            print(f"WARN  skip {code} — position not in SQLite", file=sys.stderr)
            continue
        for action in caps:
            rows.append((pid, "crm_email_mkt", action))
    ...
    with pg_connection() as conn:
        ...
        conn.commit()
    print(f"Seeded {len(rows)} crm_email_mkt permission rows for {list(positions.keys())}")
    return 0
