#!/usr/bin/env bash
# Deploy Market Research OS P17 — P0–P16 stack + portal theme QoQ/YoY delta.
#
# Forward path: 1/3 DDL (P0–P7 + P10 + P11 + P13) → 2/3 api → 3/3 portal-web.
# P17 rebuilds portal-web (like P15); ops-web unchanged.
#
# Merge this branch to main before VPS. APPLY=1 pulls origin main only.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p17_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p17_vps.sh --local
#
...
run_local() {
  echo "== Market Research P17 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== Market Research P17 deploy complete =="
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p17_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p17_vps.sh --local"
fi
