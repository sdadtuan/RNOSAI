#!/usr/bin/env bash
# Deploy HR Employee File P3 — insurance register (HR-UC-003).
set -euo pipefail
...
run_local() {
  echo "== HR Employee File P3 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  npm run build
  npm test -- --testPathPattern='hr-employee-file|hr-insurance|hr-labor-contract' --passWithNoTests --no-coverage
  ...
  echo "== HR Employee File P3 deploy complete =="
  echo "UAT: bash scripts/smoke_hr_employee_file_p3.sh"
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_hr_employee_file_p3_vps.sh --local"
else
