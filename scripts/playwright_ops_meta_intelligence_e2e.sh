#!/usr/bin/env bash
# Playwright E2E — ops-web /meta/intelligence (requires Nest + ops-web running)
set -euo pipefail
...
export NEXT_PUBLIC_PTT_META_ANOMALY_ENABLED="${NEXT_PUBLIC_PTT_META_ANOMALY_ENABLED:-1}"
export NEXT_PUBLIC_PTT_META_ROAS_ENABLED="${NEXT_PUBLIC_PTT_META_ROAS_ENABLED:-1}"
...
npm run test:e2e -- e2e/meta-intelligence.spec.ts
