#!/usr/bin/env bash
# Deploy Market Research OS P27 — RAG default excludes stale hits.
#
# Forward path: 1/2 DDL (P0–P23 incl. P20 fail-soft) → 2/2 api.
# P27 does NOT rebuild ops-web or portal-web (API only). No P27 DDL.
#
# Do NOT set RESEARCH_RAG_ENABLED / OpenAI embed / pgvector / Talkwalker flags.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p27_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p27_vps.sh --local
set -euo pipefail
...
run_local() {
  echo "== Market Research P27 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== Market Research P27 deploy complete =="
  echo "UAT: bash scripts/smoke_market_research_p27.sh"
}
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p27_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p27_vps.sh --local"
fi
