#!/usr/bin/env bash
# Deploy Market Research OS P22 — P0–P21 stack + staff RAG stale banner.
#
# Forward path: 1/3 DDL (P0–P7 + P10 + P11 + P13 + P20 fail-soft + P21) → 2/3 api → 3/3 ops-web.
# P22 does NOT rebuild portal-web (API + ops-web only). No P22 DDL.
#
# Merge this branch to main before VPS. APPLY=1 pulls origin main only.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p22_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p22_vps.sh --local
#
# Do NOT flip RAG / OpenAI embed / pgvector flags by default.
set -euo pipefail
...
run_local() {
  echo "== Market Research P22 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 1/3 Apply Market Research P0–P7 + P10 + P11 + P13 + P20 fail-soft + P21 DDL =="
  ...
  echo "== Market Research P22 deploy complete =="
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p22_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p22_vps.sh --local"
fi
