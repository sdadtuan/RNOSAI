#!/usr/bin/env bash
# Deploy Market Research OS P40 — staff RAG re-embed panel (RES-UC-102).
#
# Forward path: 1/3 DDL (P0–P38) → 2/3 api → 3/3 ops-web.
# P40 does NOT rebuild portal-web or worker.
...
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p40_vps.sh --local
set -euo pipefail
...
run_local() {
  echo "== Market Research P40 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 1/3 Apply Market Research P0–P7 + P10 + P11 + P13 + P20 + P21 + P23 + P36 + P38 DDL =="
  ...
  echo "== Market Research P40 deploy complete =="
  echo "UAT: bash scripts/smoke_market_research_p40.sh"
  echo "Panel visible only when staging rag_openai_embed_enabled + crm_research.configure"
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p40_vps.sh --local"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p40_vps.sh --local"
fi
