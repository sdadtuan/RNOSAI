#!/usr/bin/env bash
# Deploy WIN-4-C (OPA policy + CPL digest + budget cards) on VPS.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_win4c_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_win4c_vps.sh --local
set -euo pipefail
...
run_local() {
  cd "$ROOT"
  echo "== WIN-4-C deploy @ $(git rev-parse --short HEAD) =="
  ...
  bash "$ROOT/scripts/deploy_opa_bundle.sh"

  echo "== Nest ptt-crm-api =="
  cd "$ROOT/services/ptt-crm-api"
  npm ci
  npm run build
  npm test -- --testPathPattern="policy\\.spec|field-level|client-scope-modules"
  ...
  echo "== WIN-4-C UAT =="
  bash "$ROOT/scripts/run_win4c_uat.sh" || true

  echo "== WIN-4-C deploy complete =="
  echo "Flags: STAFF_POLICY_OPA=1 NEXT_PUBLIC_WIN_POLICY_OPA=1 NEXT_PUBLIC_WIN_CPL_DIGEST=1"
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_win4c_vps.sh --local"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_win4c_vps.sh --local"
fi
