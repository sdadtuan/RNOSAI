#!/usr/bin/env bash
# Deploy Market Research OS P9 — P0–P8 stack + SparkToro live HTTP (no DDL).
#
# P9 has no DDL file. Forward path: 1/4 DDL → 2/4 api → 3/4 ops-web → 4/4 worker.
# P9 does not touch portal — do NOT rebuild portal-web.
#
# Merge this branch to main before VPS. APPLY=1 pulls origin main only.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p9_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p9_vps.sh --local
#
# Do NOT flip flags by default. Explicit opt-in only:
#   bash scripts/deploy_market_research_p9_vps.sh --local --enable-flags
...
#
# Staging UAT SparkToro live: PO sets manually in runtime.env, then restart api+worker:
...
run_local() {
  echo "== Market Research P9 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 1/4 Apply Market Research P0 + P1 + P2 + P3 + P4 + P5 + P6 + P7 DDL (P7 DDL last) =="
  ...
  echo "== Market Research P9 deploy complete =="
  echo "SparkToro not enabled by this script. Staging: PO sets RESEARCH_SPARKTORO_ENABLED + SPARKTORO_API_KEY manually."
  echo "UAT: bash scripts/smoke_market_research_p9.sh"
}
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p9_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p9_vps.sh --local"
  echo "Do not set RESEARCH_SPARKTORO_ENABLED or SPARKTORO_API_KEY on prod deploy."
fi
