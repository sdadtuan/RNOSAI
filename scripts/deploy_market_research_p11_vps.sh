#!/usr/bin/env bash
# Deploy Market Research OS P11 — P0–P10 stack + OpenAI embed optional + DDL embed_model/dims.
#
# Forward path: 1/4 DDL (P0–P7 + P10 + P11) → 2/4 api → 3/4 ops-web → 4/4 worker.
# P11 does not touch portal — do NOT rebuild portal-web.
#
# Merge this branch to main before VPS. APPLY=1 pulls origin main only.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p11_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p11_vps.sh --local
#
# Do NOT flip flags by default. Explicit opt-in only:
#   bash scripts/deploy_market_research_p11_vps.sh --local --enable-flags
...
#
# Staging UAT OpenAI embed (manual, after PO DPA):
#   RESEARCH_RAG_ENABLED=1
#   RESEARCH_RAG_OPENAI_EMBED_ENABLED=1
#   # OPENAI_API_KEY already in runtime.env if Whisper/deep enabled
...
run_local() {
  echo "== Market Research P11 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 1/4 Apply Market Research P0 + P1 + P2 + P3 + P4 + P5 + P6 + P7 + P10 + P11 DDL =="
  ...
  echo "== Market Research P11 deploy complete =="
  echo "OpenAI embed not enabled by this script. Staging: PO sets RESEARCH_RAG_ENABLED + RESEARCH_RAG_OPENAI_EMBED_ENABLED manually."
  echo "UAT: bash scripts/smoke_market_research_p11.sh"
}
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p11_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p11_vps.sh --local"
  echo "Do not set RESEARCH_RAG_ENABLED, RESEARCH_RAG_OPENAI_EMBED_ENABLED, or OPENAI_API_KEY on prod deploy."
fi
