#!/usr/bin/env bash
# Deploy KPI Hub — DDL + RBAC seed + ptt-crm-api tests + ops-web build.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_kpi_hub_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_kpi_hub_vps.sh --local
set -euo pipefail
...
run_local() {
  echo "== KPI Hub deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 2/5 ptt-crm-api build + kpi-hub tests =="
  ...
  echo "== 3/5 ops-web unit (kpi-hub) =="
  cd "$ROOT/services/ops-web"
  npm ci
  npm run test:unit -- src/lib/kpi-hub-nav.spec.ts src/lib/kpi-hub-status.spec.ts
  ...
  echo "OK  KPI Hub deployed"
  echo ""
  echo "Optional E2E (staff needs crm_kpi_hub caps):"
  echo "  cd services/ops-web && npx playwright test e2e/kpi-hub.spec.ts"
...
ssh "$VPS_USER@$VPS_HOST" "cd '$VPS_ROOT' && git pull --ff-only origin main && bash scripts/deploy_kpi_hub_vps.sh --local"
