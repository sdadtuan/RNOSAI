#!/usr/bin/env bash
# Deploy Market Research OS P35 — portal conjoint lite read-only (RES-UC-096).
#
# Forward path: 1/3 DDL (P0–P23) → 2/3 api → 3/3 portal-web.
# P35 does NOT rebuild ops-web. No P35 DDL.
# Flags untouched (prod-safe).
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p35_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p35_vps.sh --local
set -euo pipefail
...
run_local() {
  echo "== Market Research P35 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 1/3 Apply Market Research P0–P7 + P10 + P11 + P13 + P20 + P21 + P23 DDL =="
  ...
  echo "== Market Research P35 deploy complete =="
  echo "UAT: bash scripts/smoke_market_research_p35.sh"
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p35_vps.sh --local"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p35_vps.sh --local"
fi
