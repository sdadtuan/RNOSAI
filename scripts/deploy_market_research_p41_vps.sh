#!/usr/bin/env bash
# Deploy Market Research OS P41 — portal report list stale badge (RES-UC-103).
#
# Forward path: 1/3 DDL (P0–P38) → 2/3 api → 3/3 portal-web.
# P41 does NOT rebuild ops-web or worker.
...
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p41_vps.sh --local
set -euo pipefail
...
run_local() {
  echo "== Market Research P41 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== Market Research P41 deploy complete =="
  echo "UAT: bash scripts/smoke_market_research_p41.sh"
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p41_vps.sh --local"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p41_vps.sh --local"
fi
