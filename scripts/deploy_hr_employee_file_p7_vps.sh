#!/usr/bin/env bash
# Deploy HR Employee File P7 — device attendance (CSV/ADMS, rollup ngày) (HR-UC-007).
set -euo pipefail
...
run_local() {
  echo "== HR Employee File P7 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"

  echo "== 1/2 apply DDL + ptt-crm-api =="
  bash "$ROOT/scripts/apply_pg_ddl_hr_employee_file_p7.sh"
  cd "$ROOT/services/ptt-crm-api"
  npm ci
  export NODE_OPTIONS="${NODE_OPTIONS:---max-old-space-size=2048}"
  export PTT_HR_EMPLOYEE_FILE="${PTT_HR_EMPLOYEE_FILE:-1}"
  npm run build
  npm test -- --testPathPattern='hr-employee-file|hr-attendance|hr-doc-wallet|hr-staff-p5' --passWithNoTests --no-coverage
  ...
  echo "== HR Employee File P7 deploy complete =="
  echo "UAT: bash scripts/smoke_hr_employee_file_p7.sh"
}
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh -o ServerAliveInterval=30 -o ServerAliveCountMax=120 \
    "${VPS_USER}@${VPS_HOST}" \
    "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_hr_employee_file_p7_vps.sh --local"
else
