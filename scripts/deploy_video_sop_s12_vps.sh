#!/usr/bin/env bash
# Video SOP S12 — OpenAI Responses + Leonardo v2 + webhook on VPS
set -euo pipefail
...
run_local() {
  cd "$ROOT"
  echo "== Video SOP S12 @ $(git rev-parse --short HEAD 2>/dev/null || echo unknown) =="
  ...
  echo "== 1/5 Apply S12 PG DDL (optional index) =="
  bash "$ROOT/scripts/apply_pg_ddl_vd_sop_s12.sh"

  echo "== 2/5 Build + restart Nest API =="
  if [[ -d "$ROOT/services/ptt-crm-api" ]]; then
    BUILD="$ROOT/.build-ptt-crm-api-vd-s12"
    rm -rf "$BUILD"
    mkdir -p "$BUILD"
  ...
  echo "== 4/5 Smoke S12 + S11 regression =="
  ...
  bash "$ROOT/scripts/smoke_video_sop_s12.sh"
  bash "$ROOT/scripts/smoke_video_sop_s11.sh"

  echo "== 5/5 S12 adapter plan present =="
  test -f "$ROOT/docs/superpowers/plans/2026-08-21-video-sop-l5-adapters.md"

  echo "== Video SOP S12 deploy complete =="
  echo "Webhook: POST /api/v1/vd/webhooks/leonardo"
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_video_sop_s12_vps.sh --local"
else
