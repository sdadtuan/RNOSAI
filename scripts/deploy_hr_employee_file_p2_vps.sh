#!/usr/bin/env bash
# Deploy HR Employee File P2 — labor contracts (HR-UC-002).
set -euo pipefail
...
run_local() {
  echo "== HR Employee File P2 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  npm run build
  npm test -- --testPathPattern='hr-employee-file|hr-labor-contract' --passWithNoTests --no-coverage
  ...
  echo "== HR Employee File P2 deploy complete =="
  echo "UAT: bash scripts/smoke_hr_employee_file_p2.sh"
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_hr_employee_file_p2_vps.sh --local"
else
