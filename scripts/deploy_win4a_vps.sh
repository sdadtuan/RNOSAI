#!/usr/bin/env bash
# Deploy WIN-4-A (Staff Keycloak SSO + MFA + group map) on VPS.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_win4a_vps.sh
#
# On VPS directly:
#   cd /var/www/rnosai && git pull origin main && bash scripts/deploy_win4a_vps.sh --local
set -euo pipefail
...
run_local() {
  cd "$ROOT"
  echo "== WIN-4-A deploy @ $(git rev-parse --short HEAD) =="
  ...
  echo "== DDL R4 staff SSO =="
  bash "$ROOT/scripts/apply_pg_ddl_staff_sso_r4.sh"

  echo "== Nest ptt-crm-api =="
  cd "$ROOT/services/ptt-crm-api"
  npm ci
  npm run build
  npm test -- --testPathPattern=staff-keycloak --passWithNoTests
  ...
  export STAFF_AUTH_MODE="${STAFF_AUTH_MODE:-dual}"
  ...
  echo "== WIN-4-A deploy complete =="
  echo "Flags: STAFF_AUTH_MODE=${STAFF_AUTH_MODE:-dual} NEXT_PUBLIC_WIN_SSO=1"
  echo "Set PTT_STAFF_KEYCLOAK_ISSUER in .env before SSO pilot (see docs/runbooks/keycloak-staff-auth.md)"
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_win4a_vps.sh --local"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_win4a_vps.sh --local"
fi
