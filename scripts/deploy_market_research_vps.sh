#!/usr/bin/env bash
# Deploy Market Research OS P0 — DDL + API + ops-web + worker.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull origin main && bash scripts/deploy_market_research_vps.sh --local
#
# Do NOT enable prod flags by default. Explicit opt-in:
#   bash scripts/deploy_market_research_vps.sh --local --enable-flags
set -euo pipefail
...
run_local() {
  echo "== Market Research P0 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 1/4 Apply Market Research DDL (both files) =="
  ...
  if [[ "$ENABLE_FLAGS" == "1" ]]; then
    ...
    echo "== flags OFF by default (pass --enable-flags after PO sign-off) =="
  fi
  ...
  echo "== Market Research P0 deploy complete =="
  echo "Flags stay off unless --enable-flags. UAT: bash scripts/smoke_market_research_p0.sh"
}
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_vps.sh --local"
  echo "Enable flags only with --enable-flags after PO sign-off."
fi
