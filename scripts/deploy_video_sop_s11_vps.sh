#!/usr/bin/env bash
# Video SOP S11 — L5 registry DDL + contract layer + SC-15 verified_at on VPS
set -euo pipefail
...
run_local() {
  cd "$ROOT"
  echo "== Video SOP S11 @ $(git rev-parse --short HEAD 2>/dev/null || echo unknown) =="
  ...
  echo "== 1/5 Apply S11 PG DDL (registry + job refs) =="
  bash "$ROOT/scripts/apply_pg_ddl_vd_sop_s11.sh"

  echo "== 2/5 Build + restart Nest API =="
  if [[ -d "$ROOT/services/ptt-crm-api" ]]; then
    BUILD="$ROOT/.build-ptt-crm-api-vd-s11"
    rm -rf "$BUILD"
    mkdir -p "$BUILD"
  ...
  echo "== 3/5 Build + publish ops-web (SC-15 model_key / verified_at) =="
  ...
  echo "== 4/5 Smoke S11 + S10 regression =="
  ...
  bash "$ROOT/scripts/smoke_video_sop_s11.sh"
  bash "$ROOT/scripts/smoke_video_sop_s10.sh"
  bash "$ROOT/scripts/smoke_video_sop_dual.sh"

  echo "== 5/5 L5 design doc present =="
  test -f "$ROOT/docs/superpowers/specs/2026-08-21-video-sop-l5-adapter-design.md"

  echo "== Video SOP S11 deploy complete =="
  echo "Admin: /admin/video/providers · Dashboard: /crm/video/dashboard?lifecycle_id={id}"
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_video_sop_s11_vps.sh --local"
else
