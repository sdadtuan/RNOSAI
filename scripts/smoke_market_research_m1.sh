...
if [[ "$HTTP_CODE" != "200" ]]; then
  echo "SKIP live M1 rubric — health not 200 (flag off, API down, or unauthenticated path)."
  echo "Contract:"
...
if [[ -z "${ACCESS_TOKEN:-}" || -z "${CLIENT_ID:-}" ]]; then
  echo "SKIP live M1 rubric — set ACCESS_TOKEN and CLIENT_ID"
  exit 0
fi
...
[[ "$HTTP_CODE" == "201" ]]
PID="$(python3 -c "import json; print(json.load(open('/tmp/mr_m1_create.json'))['project']['id'])")"

echo "==> POST /projects/$PID/sources"
HTTP_CODE="$(curl -sS -o /tmp/mr_m1_source.json -w '%{http_code}' "${AUTH[@]}" \
...
[[ "$HTTP_CODE" == "201" ]]
SID="$(python3 -c "import json; print(json.load(open('/tmp/mr_m1_source.json'))['id'])")"

echo "==> POST /projects/$PID/evidence"
HTTP_CODE="$(curl -sS -o /tmp/mr_m1_ev.json -w '%{http_code}' "${AUTH[@]}" \
...
[[ "$HTTP_CODE" == "201" ]]
EID="$(python3 -c "import json; print(json.load(open('/tmp/mr_m1_ev.json'))['id'])")"

echo "==> POST /evidence/$EID/verify"
HTTP_CODE="$(curl -sS -o /tmp/mr_m1_ver.json -w '%{http_code}' "${AUTH[@]}" \
  -X POST "$API_BASE/api/v1/research/evidence/$EID/verify")"
[[ "$HTTP_CODE" == "200" || "$HTTP_CODE" == "201" ]]

echo "==> POST /projects/$PID/insights with rubric"
HTTP_CODE="$(curl -sS -o /tmp/mr_m1_insight.json -w '%{http_code}' "${AUTH[@]}" \
...
echo "http=$HTTP_CODE body=$(cat /tmp/mr_m1_insight.json)"
[[ "$HTTP_CODE" == "201" ]]
IID="$(python3 -c "import json; print(json.load(open('/tmp/mr_m1_insight.json'))['id'])")"

echo "==> POST /insights/$IID/attach-evidence"
HTTP_CODE="$(curl -sS -o /tmp/mr_m1_attach.json -w '%{http_code}' "${AUTH[@]}" \
  -X POST "$API_BASE/api/v1/research/insights/$IID/attach-evidence" \
  -d "{\"evidence_ids\":[$EID]}")"
[[ "$HTTP_CODE" == "200" || "$HTTP_CODE" == "201" ]]

echo "==> POST /insights/$IID/submit-review"
HTTP_CODE="$(curl -sS -o /tmp/mr_m1_submit.json -w '%{http_code}' "${AUTH[@]}" \
  -X POST "$API_BASE/api/v1/research/insights/$IID/submit-review" \
  -d '{"confidence_json":{"S":4,"F":4,"T":4,"A":4,"R":4}}')"
echo "http=$HTTP_CODE body=$(cat /tmp/mr_m1_submit.json)"
[[ "$HTTP_CODE" == "200" || "$HTTP_CODE" == "201" ]]
python3 - <<'PY'
import json
row=json.load(open('/tmp/mr_m1_submit.json'))
