#!/usr/bin/env bash
# Deploy Market Research OS P23 — P0–P22 stack + Talkwalker stub.
#
# Forward path: 1/3 DDL (P0–P7 + P10 + P11 + P13 + P20 fail-soft + P21 + P23) → 2/3 api → 3/3 ops-web.
# P23 does NOT rebuild portal-web (API + ops-web only).
#
...
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p23_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p23_vps.sh --local
set -euo pipefail
...
run_local() {
  echo "== Market Research P23 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== Market Research P23 deploy complete =="
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p23_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p23_vps.sh --local"
fi
