#!/usr/bin/env bash
# Deploy Market Research OS P36 — IVFFlat fail-soft + live Talkwalker (RES-UC-097 / RES-UC-098).
#
# Forward path: 1/2 DDL (P0–P36) → 2/2 api.
# P36 does NOT rebuild ops-web or portal-web.
# Flags untouched (prod-safe).
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p36_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p36_vps.sh --local
set -euo pipefail
...
run_local() {
  echo "== Market Research P36 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 1/2 Apply Market Research P0–P7 + P10 + P11 + P13 + P20 + P21 + P23 + P36 DDL =="
  ...
  echo "== Market Research P36 deploy complete =="
  echo "UAT: bash scripts/smoke_market_research_p36.sh"
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p36_vps.sh --local"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p36_vps.sh --local"
fi
