#!/usr/bin/env bash
# Deploy HR Employee File P8 — GPS geofence + self punch + review (HR-UC-008).
set -euo pipefail
...
run_local() {
  echo "== HR Employee File P8 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"

  echo "== 1/2 apply DDL + ptt-crm-api =="
  bash "$ROOT/scripts/apply_pg_ddl_hr_employee_file_p8.sh"
  ...
  echo "== HR Employee File P8 deploy complete =="
  echo "UAT: bash scripts/smoke_hr_employee_file_p8.sh"
}
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh -o ServerAliveInterval=30 -o ServerAliveCountMax=120 \
    "${VPS_USER}@${VPS_HOST}" \
    "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_hr_employee_file_p8_vps.sh --local"
else
