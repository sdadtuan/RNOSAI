#!/usr/bin/env bash
# Deploy Market Research OS P13 — P0–P12 stack + RAG re-embed backfill job.
#
# Forward path: 1/4 DDL (P0–P7 + P10 + P11 + P13) → 2/4 api → 3/4 ops-web → 4/4 worker.
# P13 does NOT rebuild portal-web (worker + API only).
#
# Merge this branch to main before VPS. APPLY=1 pulls origin main only.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p13_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p13_vps.sh --local
#
...
run_local() {
  echo "== Market Research P13 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 1/4 Apply Market Research P0–P7 + P10 + P11 + P13 DDL =="
  ...
  echo "== Market Research P13 deploy complete =="
  echo "Re-embed requires staging RAG + OpenAI embed flags. UAT: bash scripts/smoke_market_research_p13.sh"
}
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p13_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p13_vps.sh --local"
fi
