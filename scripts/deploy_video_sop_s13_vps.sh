#!/usr/bin/env bash
# Video SOP S13 — Runway live + Kling VIA_LEONARDO + poller cadence on VPS
set -euo pipefail
...
run_local() {
  cd "$ROOT"
  echo "== Video SOP S13 @ $(git rev-parse --short HEAD 2>/dev/null || echo unknown) =="
  ...
  echo "== 1/5 Apply S13 PG DDL (no-op) =="
  bash "$ROOT/scripts/apply_pg_ddl_vd_sop_s13.sh"

  echo "== 2/5 Build + restart Nest API =="
  if [[ -d "$ROOT/services/ptt-crm-api" ]]; then
    BUILD="$ROOT/.build-ptt-crm-api-vd-s13"
    rm -rf "$BUILD"
    mkdir -p "$BUILD"
  ...
  echo "== 4/5 Smoke S13 + S12 regression =="
  ...
  echo "== 5/5 S13 plan present =="
  test -f "$ROOT/docs/superpowers/plans/2026-08-21-video-sop-l5-adapters.md"

  echo "== Video SOP S13 deploy complete =="
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_video_sop_s13_vps.sh --local"
else
