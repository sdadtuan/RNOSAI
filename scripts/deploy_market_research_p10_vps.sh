#!/usr/bin/env bash
# Deploy Market Research OS P10 — P0–P9 stack + Qualtrics live HTTP + DDL qualtrics.
#
# Forward path: 1/4 DDL (P0–P7 + P10) → 2/4 api → 3/4 ops-web → 4/4 worker.
# P10 does not touch portal — do NOT rebuild portal-web.
#
# Merge this branch to main before VPS. APPLY=1 pulls origin main only.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p10_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p10_vps.sh --local
#
# Do NOT flip flags by default. Explicit opt-in only:
#   bash scripts/deploy_market_research_p10_vps.sh --local --enable-flags
...
# and does NOT write QUALTRICS_API_KEY, QUALTRICS_DATACENTER, or SPARKTORO_API_KEY.
#
# Staging UAT Qualtrics live: PO sets manually in runtime.env, then restart api+worker:
...
run_local() {
  echo "== Market Research P10 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 1/4 Apply Market Research P0 + P1 + P2 + P3 + P4 + P5 + P6 + P7 + P10 DDL =="
  ...
  if [[ "$ENABLE_FLAGS" == "1" ]]; then
    echo "== flags --enable-flags (staging/UAT only; RAG + Qualtrics + SparkToro stay off) =="
    mkdir -p "$ROOT/deploy"
    touch "$ROOT/deploy/runtime.env"
    patch_runtime_env "$ROOT/deploy/runtime.env"
    patch_runtime_env "$ROOT/.env"
  else
    echo "== flags untouched (P0 on; RAG + Qualtrics + SparkToro stay off) =="
  fi

  echo "== 2/4 ptt-crm-api =="
  ...
  echo "== Market Research P10 deploy complete =="
  echo "Qualtrics not enabled by this script. Staging: PO sets RESEARCH_QUALTRICS_ENABLED + QUALTRICS_API_KEY + QUALTRICS_DATACENTER manually."
  echo "UAT: bash scripts/smoke_market_research_p10.sh"
}
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p10_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p10_vps.sh --local"
  echo "Do not set RESEARCH_QUALTRICS_ENABLED, QUALTRICS_API_KEY, or QUALTRICS_DATACENTER on prod deploy."
fi
