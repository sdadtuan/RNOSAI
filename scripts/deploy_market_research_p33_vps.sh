#!/usr/bin/env bash
# Deploy Market Research OS P33 — show published_valid_to on portal + staff (RES-UC-094).
#
# Forward path: 1/4 DDL (P0–P23) → 2/4 api → 3/4 ops-web → 4/4 portal-web.
# P33 does NOT add DDL. Flags untouched (prod-safe).
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p33_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p33_vps.sh --local
set -euo pipefail
...
run_local() {
  echo "== Market Research P33 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 1/4 Apply Market Research P0–P7 + P10 + P11 + P13 + P20 + P21 + P23 DDL =="
  ...
  echo "== Market Research P33 deploy complete =="
  echo "UAT: bash scripts/smoke_market_research_p33.sh"
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p33_vps.sh --local"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p33_vps.sh --local"
fi
