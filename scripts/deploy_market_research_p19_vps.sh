#!/usr/bin/env bash
# Deploy Market Research OS P19 — P0–P18 stack + portal RAG stale banner.
#
# Forward path: 1/3 DDL (P0–P7 + P10 + P11 + P13) → 2/3 api → 3/3 portal-web.
# P19 rebuilds portal-web (like P17); ops-web unchanged.
#
# Merge this branch to main before VPS. APPLY=1 pulls origin main only.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p19_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p19_vps.sh --local
#
...
run_local() {
  echo "== Market Research P19 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== Market Research P19 deploy complete =="
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p19_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p19_vps.sh --local"
fi
