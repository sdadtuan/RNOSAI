#!/usr/bin/env bash
# Deploy Market Research OS P31 — staff DOCX stale footer (RES-UC-092).
#
# Forward path: 1/2 DDL (P0–P23) → 2/2 api.
# P31 does NOT rebuild ops-web or portal-web (API only). No P31 DDL.
...
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p31_vps.sh --local
set -euo pipefail
...
run_local() {
  echo "== Market Research P31 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== Market Research P31 deploy complete =="
  echo "UAT: bash scripts/smoke_market_research_p31.sh"
}

if [[ "${1:-}" == "--local" ]]; then
  run_local
elif [[ "$APPLY" == "1" ]]; then
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p31_vps.sh --local"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p31_vps.sh --local"
fi
