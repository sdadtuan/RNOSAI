#!/usr/bin/env bash
# Deploy Quotation OS Wave 1 — DDL + catalog + ptt-crm-api + ops-web + portal-web.
# Do NOT export QT_AI_ENABLED=1 — AI stays off until W1 UAT signoff is green.
# Do NOT grant crm_quote caps here — seed_qt_rbac.sh is catalog-only.
# Do NOT apply Wave 2 / Wave 3 DDL.
#
# From laptop:
...
run_local() {
  echo "== Quotation OS Wave 1 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 0/6 apply QT Wave 1 DDL =="
  bash "$ROOT/scripts/apply_pg_ddl_qt.sh"

  echo "== 1/6 QT RBAC catalog (no user grants) =="
  bash "$ROOT/scripts/seed_qt_rbac.sh"

  echo "== 2/6 ptt-crm-api build + QT unit tests =="
  ...
  echo "== 3/6 ops-web QT unit tests =="
  ...
  echo "== 5/6 portal-web rebuild (public quote accept) =="
  bash "$ROOT/scripts/wave_b2_rebuild_portal_web.sh"

  echo "== 6/6 restart services (one unit per command) =="
  ...
  echo "OK  Quotation OS Wave 1 deployed (QT_AI_ENABLED not enabled; grant crm_quote via Admin RBAC)"
}
