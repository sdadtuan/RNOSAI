#!/usr/bin/env bash
# Deploy Market Research OS P6 — P0+P1+P2+P3+P4+P5+P6 DDL + API + ops-web + worker.
#
# Ordering: P6 DDL MUST apply last in step 1, before api / ops-web / worker restart.
# Forward path: 1/4 DDL → 2/4 api → 3/4 ops-web → 4/4 worker.
# P6 does not touch portal — do NOT rebuild portal-web.
#
# Merge this branch to main before VPS. APPLY=1 pulls origin main only
# (same as P5) — do not deploy an unmerged feature branch.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p6_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p6_vps.sh --local
#
# Do NOT flip flags by default (P0 already on). Explicit opt-in only:
#   bash scripts/deploy_market_research_p6_vps.sh --local --enable-flags
# --enable-flags writes P0 research flags only. It does NOT set
# RESEARCH_QUALTRICS_ENABLED=1, RESEARCH_SPARKTORO_ENABLED=1, and does NOT
# write QUALTRICS_API_KEY or SPARKTORO_API_KEY.
set -euo pipefail
...
patch_runtime_env() {
  ...
  # P0 research flags only. Never write Qualtrics / SparkToro enable or API keys.
  for kv in \
...
run_local() {
  echo "== Market Research P6 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== 1/4 Apply Market Research P0 + P1 + P2 + P3 + P4 + P5 + P6 DDL (P6 DDL last, before restarts) =="
  ...
  if [[ "$ENABLE_FLAGS" == "1" ]]; then
    echo "== flags --enable-flags (staging/UAT only; not default; Qualtrics + SparkToro stay off) =="
    mkdir -p "$ROOT/deploy"
    touch "$ROOT/deploy/runtime.env"
    patch_runtime_env "$ROOT/deploy/runtime.env"
    patch_runtime_env "$ROOT/.env"
  else
    echo "== flags untouched by default (P0 already on; Qualtrics + SparkToro stay off; pass --enable-flags only as explicit opt-in) =="
  fi
  ...
  echo "== Market Research P6 deploy complete =="
  echo "Flags not flipped unless --enable-flags. Qualtrics not enabled. SparkToro not enabled. No portal-web rebuild."
  echo "UAT: bash scripts/smoke_market_research_p6.sh"
}
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  # Merge-to-main happens before VPS. Pull origin main only (same as P5).
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p6_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p6_vps.sh --local"
