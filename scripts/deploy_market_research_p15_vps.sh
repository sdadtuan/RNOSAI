#!/usr/bin/env bash
# Deploy Market Research OS P15 — P0–P14 stack + portal theme quarter analytics.
#
# Forward path: 1/3 DDL (P0–P7 + P10 + P11 + P13) → 2/3 api → 3/3 portal-web.
# P15 rebuilds portal-web (like P12); ops-web unchanged.
#
# Merge this branch to main before VPS. APPLY=1 pulls origin main only.
#
# From laptop:
#   APPLY=1 ./scripts/deploy_market_research_p15_vps.sh
#
# On VPS:
#   cd /var/www/rnosai && git pull --ff-only origin main && bash scripts/deploy_market_research_p15_vps.sh --local
#
...
run_local() {
  echo "== Market Research P15 deploy @ $(git -C "$ROOT" rev-parse --short HEAD) =="
  cd "$ROOT"
  ...
  echo "== Market Research P15 deploy complete =="
  echo "UAT: bash scripts/smoke_market_research_p15.sh"
}
...
if [[ "${1:-}" == "--local" ]]; then
  ...
  ssh "${VPS_USER}@${VPS_HOST}" "cd ${VPS_ROOT} && git pull --ff-only origin main && bash scripts/deploy_market_research_p15_vps.sh ${REMOTE_ARGS}"
else
  echo "Dry-run. Set APPLY=1 to deploy to ${VPS_USER}@${VPS_HOST}:${VPS_ROOT}"
  echo "Or on VPS: bash scripts/deploy_market_research_p15_vps.sh --local"
fi
